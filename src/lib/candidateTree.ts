...
export function isRootCandidate(
...
export function listRootCandidates<T extends CandidateTreeRow>(
  ...
  return candidates
    .filter(c => isRootCandidate(c))
    .sort(compareCandidatesSiblingOrder);
}

export function listChildrenOf<T extends CandidateTreeRow>(
  parentId: string,
  candidates: T[]
): T[] {
  return candidates
...
export function listOrphanVariations<T extends CandidateTreeRow>(
  candidates: T[]
): T[] {
  return candidates
...
export function countVariationsForRoot(
  rootId: string,
...
export function validateParentAssignment(
  ...
  if (nextParentId === candidateId) {
    return {
      ok: false,
      message: 'Une fiche ne peut pas être son propre parent.',
    };
  }
  ...
  if (
    self &&
    isRootCandidate(self) &&
    countVariationsForRoot(candidateId, candidates) > 0
  ) {
    return {
      ok: false,
      message:
...
export function resolveIdentityForCandidateUpdate(opts: {
  ...
  if (!opts.nextParentId) {
    return {
      ...
      event_date: opts.meta.event_date?.trim()
        ? String(opts.meta.event_date).trim()
        : null,
    };
  }
...
export function postOrderDeleteIds<T extends CandidateTreeRow>(
  target: T,
  all: T[]
): string[] {
  const children = all
