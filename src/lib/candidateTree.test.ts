...
describe('candidateTree', () => {
  ...
  it('validateParentChange reflète racine avec compléments', () => {
    expect(
      validateParentChange('a', 'x', { isRoot: true, directVariationCount: 1 })
        .ok
    ).toBe(false);
    expect(
      validateParentChange('a', 'x', { isRoot: true, directVariationCount: 0 })
        .ok
    ).toBe(true);
  });
  ...
  it('eligibleRootParentsForSelect exclut soi-même', () => {
    ...
    expect(eligibleRootParentsForSelect('a', roots).map(x => x.id)).toEqual([
      'b',
    ]);
  });
