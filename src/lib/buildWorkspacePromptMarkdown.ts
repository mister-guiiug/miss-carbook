...
export function buildWorkspacePromptMarkdown(
  ...
  push('', '---', '', '## Membres', '');
  const members = bundle.members as {
    user_id: string;
    role: string;
    joined_at?: string;
  }[];
  ...
  if (cv && Object.keys(cv).length) {
    ...
    if (specs != null && typeof specs === 'object' && !Array.isArray(specs)) {
      ...
      if (door != null && door !== '')
        push(`- **Nombre de portes** : ${esc(door)}`);
      if (hp != null && hp !== '') push(`- **Puissance (ch)** : ${esc(hp)}`);
      if (fiscal != null && fiscal !== '')
        push(`- **Puissance fiscale (CV)** : ${esc(fiscal)}`);
      if (trunk != null && trunk !== '')
        push(`- **Volume du coffre (L)** : ${esc(trunk)}`);
      if (box != null && String(box).trim() !== '')
        push(`- **Boîte de vitesses** : ${esc(box)}`);
      if (col != null && String(col).trim() !== '')
        push(`- **Couleur extérieure** : ${esc(col)}`);
      const known = new Set([
        'lengthMm',
        'widthMm',
        'heightMm',
  ...
  if (!cands.length) {
    ...
    for (const c of cands) {
      ...
      const links = parseManufacturerLinksFromDb(
        c.manufacturer_links,
        c.manufacturer_url
      );
      const linksLine =
        links.length === 0
      ...
      push(
        ...
        `- **Kilométrage** : ${
          c.mileage_km != null
            ? `${esc(formatMileageKmDisplay(c.mileage_km))} km`
            : '—'
        }`,
      ...
      if (
        specs != null &&
        typeof specs === 'object' &&
        Object.keys(specs as object).length
      ) {
        push(
  ...
  if (!evals.length) {
    ...
    for (const e of evals) {
      ...
      push(
        `- **${rl}** × **${cl}** → ${st}${e.note ? ` — _${esc(e.note)}_` : ''}`
      );
    }
  }

  push('', '---', '', '## Votes de priorité (MoSCoW) par exigence', '');
  const votes = bundle.votes as {
    requirement_id: string;
    user_id: string;
    vote: string;
  }[];
  ...
  if (legacy?.body != null && String(legacy.body).trim()) {
    push(
      '',
      '_Bloc-notes partagé (historique) :_',
      '',
      esc(String(legacy.body))
    );
  }
  ...
  if (!presets.length) {
    ...
    for (const p of presets) {
      push(
        `- **${esc(p.name)}** : \`${esc(JSON.stringify(p.criteria_keys))}\``
      );
    }
  }
  ...
  if (!atts.length) {
    ...
    push(
      `_${atts.length} fichier(s) — pas de contenu binaire dans cet export._`,
      ''
    );
    for (const a of atts) {
      ...
      push(
        `- **${esc(cl)}** — type ${esc(a.mime_type)} — ${esc(a.size_bytes)} octets`
      );
    }
  }
  ...
  if (!actSlice.length) {
    ...
    if (acts.length > actSlice.length) {
      push(
        '',
        `_(… ${acts.length - actSlice.length} entrée(s) supplémentaires non affichées.)_`
      );
    }
  }
