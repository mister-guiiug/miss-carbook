...
export function NotepadTab({
  ...
  const [peerOrderOverride, setPeerOrderOverride] = useState<string[] | null>(
    null
  );
  ...
  const { myMember, orderedOthers } = useMemo(() => {
    ...
    const others = myId
      ? members.filter(m => m.user_id !== myId)
      : [...members];
  ...
  const canReorderOthers = Boolean(
    canWrite && myId && orderedOthers.length > 1 && !reordering
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      for (const p of profs ?? [])
        names[(p as { id: string }).id] = (
          p as { display_name: string }
        ).display_name;
      setMembers(
        ids.map(id => ({
          user_id: id,
          display_name: names[id] ?? id.slice(0, 8),
        }))
      );
    ...
    onDrop:
      canDrag && ro
        ? (e: DragEvent<HTMLDivElement>) => {
            ...
            setDragOverId(null);
            if (draggedId)
              onDropReorderOthers(m.user_id, draggedId, ro.siblingIds);
          }
        : undefined,
  });
    ...
    return (
      <div
        ...
        className={`card stack notepad-peer-inline${
          canDrag && draggingId === m.user_id
            ? ' notepad-accordion-item--dragging'
            : ''
        ...
        <div className="row notepad-peer-inline-head">
          ...
          >
            ...
            {dateLabel ? (
              <>
                <span
                  className="notepad-peer-inline-sep muted"
                  aria-hidden="true"
                >
                  ·
                </span>
                <time
                  className="notepad-peer-inline-date muted"
          ...
          >
            <span className="notepad-accordion-header-text">
              ...
              <span className="muted notepad-accordion-status">
                <span
                  className="notepad-accordion-status-sep"
                  aria-hidden="true"
                >
                  ·
                </span>
