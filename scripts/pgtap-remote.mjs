 ...
 *
 * Repris de `mister-miss-koh/scripts/pgtap-remote.mjs` (05/09/2026), avec une
 * addition : `--with <fichier.sql>`.
 *
 * POURQUOI `--with`. `supabase test db` exige `pg_prove` dans un conteneur, et
 * Docker ne démarre pas sur le poste. `supabase db query --linked` exécute bien
 * le fichier mais ne renvoie que le dernier jeu de lignes : les verdicts
 * intermédiaires de pgTAP se perdent. Ce lanceur réécrit donc chaque assertion
 ...
 * reste.
 *
 * `--with` insère un fichier de migration juste après le `begin;` du test. La
 * migration est donc CRÉÉE puis DÉFAITE dans la même transaction : on éprouve
 * une fonction qui n'est pas encore déployée, sans rien appliquer à la base.
 * Une fois la migration poussée, relancer SANS `--with` teste le déployé.
 *
 * Prérequis : `supabase link` fait, et `SUPABASE_ACCESS_TOKEN` dans
 * l'environnement (ou une session `supabase login`).
 *
 * Usage : node scripts/pgtap-remote.mjs <fichier.test.sql> [--with <chemin.sql>]
 */
...
const prelude =
  withIndex === -1 ? '' : readFileSync(argv[withIndex + 1], 'utf8');
const name = argv.filter((a, i) => i !== withIndex && i !== withIndex + 1)[0];
...
const file = join(mkdtempSync(join(tmpdir(), 'carbook-pgtap-')), 'remote.sql');
